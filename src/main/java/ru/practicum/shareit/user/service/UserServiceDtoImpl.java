...
public class UserServiceDtoImpl implements UserServiceDto {
    ...
    private void checkEmail(User user) {
        boolean isEmailNotUnique = userServiceDao.findAll()
                .stream().anyMatch(thisUser -> thisUser.getEmail().equals(user.getEmail())
                        && !thisUser.getId().equals(user.getId()));
        if (isEmailNotUnique) {
            throw new NotUniqueEmailException("Электронная почта уже существует");
        }
    }

    private boolean isUserInMemory(Long userId) {
        return userServiceDao.findAll().stream().anyMatch(user -> user.getId().equals(userId));
    }
}